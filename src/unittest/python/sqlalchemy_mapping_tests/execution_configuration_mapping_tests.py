...
class ExecutionConfigurationMappingTest(MappingTest):
    ...
    def test_crud_execution_configuration(self):
        ...
        execution_configuration = ExecutionConfiguration("ls", "slurm:sbatch", "-X")
        ...
        execution_configuration = db.session.query(ExecutionConfiguration).filter_by(command='ls').first()
        self.assertIsNotNone(execution_configuration.id)
        self.assertEquals("ls", execution_configuration.command)
        self.assertEquals("slurm:sbatch", execution_configuration.execution_type)
        self.assertEquals("-X", execution_configuration.parameters)

        # We check that we can update the application
        execution_configuration.parameters = '-X1'
        db.session.commit()
        execution_configuration_2 = db.session.query(ExecutionConfiguration).filter_by(command='ls').first()
        self.assertEquals(execution_configuration.id, execution_configuration_2.id)
        self.assertEquals("-X1", execution_configuration.parameters)

        # We check the deletion
        db.session.delete(execution_configuration_2)
        count = db.session.query(ExecutionConfiguration).filter_by(command='ls').count()
